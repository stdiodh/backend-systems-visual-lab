...
- Next Question: DB가 아닌 외부 API나 연동 서비스가 문제라면 timeout, retry, circuit breaker를
  어떻게 봐야 하는가?
...
- Next Question: 비동기로 분리해도 여러 작업자가 같은 데이터를 수정하면 데이터가 꼬인다.
  동시성은 어떻게 막아야 하는가?
...
- Next Question: 동시성을 제어해도 IO 대기가 길면 서버 자원이 낭비된다. IO 병목은 어떻게 볼
  것인가?
...
- Next Question: 성능과 안정성만으로는 충분하지 않다. 인증, 인가, 암호화, 감사 로그 같은 보안
  지식은 어떻게 적용해야 하는가?
...
- Next Question: 서비스를 안전하게 만들었더라도 서버에서 프로세스, 디스크, 권한, 시간,
  네트워크를 확인할 줄 알아야 한다.
...
- Next Question: 서버 내부를 확인할 수 있어도 요청이 서버까지 어떻게 오는지 모르면 네트워크
  문제를 해결하기 어렵다.
...
- Next Question: 네트워크와 서버 흐름을 이해했다면, 서비스를 어떤 구조와 설계 패턴으로 나눌지
  판단해야 한다.
...
- Next Question: 성능과 확장성을 고려하다 보면 RDB 외 저장소가 필요한지 판단해야 한다. NoSQL은
  언제 고려해야 하는가?
...
- Next Question: 분산 환경에서 공유 자원을 제어해야 할 때 DB 기반 분산 잠금을 어떻게 구현할 수
  있는가?
...
- Detail Doc:
  [appendix-c-db-distributed-lock.md](../sequences/appendix-c-db-distributed-lock.md)
