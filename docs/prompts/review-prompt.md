...
7. 첫 화면이 평범한 다크 관리자 대시보드가 아니라 Hero, Codex Terminal Panel, Chapter Progress Rail 조합을 따르는가
8. Chapter Rail이 단순 목차가 아니라 진행 상태와 코드명을 가진 Chapter Progress Rail로 보이는가
9. Incident Panel이 각 장의 첫 스크롤 안에 보이는가
10. 중앙 레포의 시퀀스 상태나 서브모듈 포인터를 건드리지 않았는가
11. 사용자가 모르면 질문하라고 한 영역을 추측으로 채우지 않았는가
