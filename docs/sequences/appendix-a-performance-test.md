...
이 부록은 앞선 성능/DB/외부 연동/아키텍처 개선이 실제로 효과가 있는지 측정하는 성능 테스트 기본기를 정리한다.

**이전 시퀀스와의 연결**  
11장에서 구조와 패턴을 선택했다. 이제 그 구조가 어느 정도 부하를 견디는지 측정한다.

**다음 시퀀스로 넘길 질문**  
성능과 확장성을 고려하다 보면 RDB 외 저장소가 필요한지 판단해야 한다. NoSQL은 언제 고려해야 하는가?
...
서비스 오픈 전 “동시 사용자 1,000명을 버틸 수 있나요?”라는 질문을 받았다. 하지만 어떤 테스트를 해야 하는지, 어떤 지표를 봐야 하는지, 언제 실패라고 판단해야 하는지 기준이 없다.
