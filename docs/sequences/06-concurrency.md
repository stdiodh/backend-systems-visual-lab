...
이 시퀀스는 여러 요청, 여러 스레드, 여러 프로세스가 같은 데이터에 접근할 때 발생하는 문제를 다룬다. 서버 메모리 공유 문제부터 DB 락, optimistic/pessimistic lock, single writer까지 연결한다.

**이전 시퀀스와의 연결**  
05장에서 비동기 처리로 작업을 분리했다. 이제 여러 consumer나 요청이 같은 데이터를 동시에 수정할 때 생기는 문제를 다룬다.
...
재고가 1개 남은 상품을 두 사용자가 동시에 주문한다. 두 요청 모두 재고가 있다고 판단하고 주문을 성공 처리한다. 결과적으로 재고가 -1이 되거나 같은 쿠폰이 중복 사용된다.
...
**흔한 오해**  
낙관적 락은 대기하지 않으니 항상 좋다고 생각하는 것이다. 충돌 시 재시도와 사용자 응답 정책이 필요하다.
...
**흔한 오해**  
단일 스레드는 느리기만 한 방식이라고 생각하는 것이다. 특정 hot key에는 안정적인 선택이 될 수 있다.
