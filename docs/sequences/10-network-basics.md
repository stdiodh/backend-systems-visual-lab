...
다룬다. 노드, 네트워크, 라우터, IP, 도메인, NAT, VPN, TCP/UDP/QUIC을 시각 흐름으로 연결한다.
...
사용자는 API가 안 된다고 말하지만 서버 프로세스는 정상이고 로그에도 요청이 없다. DNS, NAT,
방화벽, 라우팅, VPN 중 어디서 요청이 막혔는지 알 수 없다.
...
**흔한 오해**
VPN에 연결하면 모든 네트워크 문제가 해결된다고 생각하는 것이다. 라우팅과 DNS가 여전히 문제일 수
있다.
...
**정의**
전송 계층 프로토콜이다. TCP는 연결 기반, UDP는 비연결 기반, QUIC은 UDP 위에서 동작하며 연결
설정과 전송 특성이 다르다.
