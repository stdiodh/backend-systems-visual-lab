...
기존 시퀀스 실습을 마친 뒤, 실무 백엔드 시스템에서 자주 만나는 주제를 Visual Lab으로
학습한다.

다루는 주제는 성능, DB, 외부 연동, 비동기, 동시성, IO, 보안, 서버, 네트워크,
아키텍처다.
...
중앙 레포의 `docs/sequences/*` 완료 상태, 기존 서브모듈 포인터, 기존 시퀀스별
Visual Lab 구현 위치는 변경하지 않는다.
...
사이트 구현자는 `docs/curriculum/curriculum-map.md`만 보고 구현하지
않는다.
