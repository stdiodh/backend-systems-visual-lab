...
권장 레포 이름은 `aandi-backend-systems-visual-lab`이다. 이 랩은 단일 시퀀스 보조 페이지가 아니라, 실무 백엔드 시스템 전반을 하나의 흐름으로 학습하기 위한 별도 학습 공간이다.

## 목적

기존 시퀀스 실습을 마친 뒤, 실무 백엔드 시스템에서 자주 만나는 성능, DB, 외부 연동, 비동기, 동시성, IO, 보안, 서버, 네트워크, 아키텍처 주제를 Visual Lab으로 학습한다.
...
중앙 레포는 이 저장소를 안내 링크로만 참조한다. 중앙 레포의 `docs/sequences/*` 완료 상태, 기존 서브모듈 포인터, 기존 시퀀스별 Visual Lab 구현 위치는 변경하지 않는다.
